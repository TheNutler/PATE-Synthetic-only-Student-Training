...
class Job:
    ...
    def create_job_clear_sequential(
        self,
        nb_samples: int,
        differential_privacy_parameter: float,
        num_teachers: int,
        ...
        Args:
            ...
            num_teachers: Total number of teachers
            batch_size: Number of teachers per batch
            api_host: API host
            api_port: API port
            base_port: Starting RPyC port for teachers
        """
        # Import teacher_manager for sequential batching
        from pathlib import Path
        try:
        ...
        for batch_num in range(num_batches):
            ...
            teacher_processes = start_teachers(
                ...
                base_port=base_port + batch_start,
                data_format=self.data_type,
                orchestrator_host=api_host,
            ...
            try:
                consumer: KafkaConsumer = utilities.connect_kafka_consumer(
                    topic_name,
                    f"orchestrator_batch_{batch_num}",
        ...
        utilities.clean_topic(topic_name)
        kafka_producer: KafkaProducer = utilities.connect_kafka_producer()
        logger.info("Publishing aggregated result")
        from alive_progress import alive_bar

        with alive_bar(len(aggregated_votes), bar="bubbles") as bar:
            for vote_id, vote in enumerate(aggregated_votes):
                utilities.publish_message(
                    kafka_producer, topic_name, vote_id, vote, encode=True
                )
                bar()
    ...
    def aggregation(
        ...
        try:
            ...
            # Note: connect_kafka_consumer already subscribes to the topic via constructor
            # No need to subscribe again - removed redundant subscribe() call
            received_votes = 0
            from alive_progress import alive_bar
