...
class ThalesDataOwner(DataOwner):
    ...
    def load_public_training_dataset(
        ...
        # Use the dataset path from input-data directory
        # For MNIST, this should point to src/input-data/MNIST (parent of MNIST/MNIST/raw)
        script_dir = Path(__file__).parent.parent  # Go up from usecases to src
        dataset_root = script_dir / "input-data" / dataset_name

        dataset = getattr(torchvision.datasets, dataset_name)(
            str(dataset_root), train=False, download=False, transform=transform
        )
